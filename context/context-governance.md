...
| :--- | :--- | :--- |
| `ai/context-common.md` | 모든 에이전트 공통 안전 원칙 | 짧게 유지, 정책 중심 |
| `ai/agent/AGENT_MANIFEST.md` | 에이전트 라우팅과 PR 게이트 | 라우팅 표와 정책만 유지 |
| `ai/context/*.summary.md` | 도메인 핵심 지식 | 1~2 화면 분량 |
| `ai/context/*.checklist.md` | 검증 항목 | 체크 가능한 항목 중심 |
| `ai/context/reverse_sync_prevention.md` | 누적 실패/재발방지 로그 | append-only 로그로 분리 |
| `ai/context/context-preprocessing-boundary.md` | 서비스 vs 시각화 전처리 경계 정의 가이드 | 아키텍처 및 샌드박스 경계 수칙 |
| `ai/evals/golden_tasks.yaml` | 벤치마크 태스크 | 측정 가능한 task/metric만 유지 |
...
* **[보관 위치 원칙]**: 모든 AI 하네스 문서 및 시스템/도메인 컨텍스트 문서는 **오직 `ai/` 폴더 및 그 하위인 `ai/context/` 디렉터리 내에만 보관**되어야 합니다.
...
* **[목적]**: 정보의 전역적이고 투명한 통합 관리 및 컨텍스트 폭주 방지, 그리고 소스 디렉터리의 본질적인 비즈니스 로직 보전을 위한 정책입니다. 만일 이외의 경로에서 발견 시 예외 없이 `ai/context/context-<도메인>.md` 명칭으로 이관 정리를 수행해야 합니다.
