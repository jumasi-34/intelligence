...
```
core/
├── common_config.py                # re-export bridge → constants/business.py
├── common_design_parameter.py      # UI 디자인 파라미터
├── common_helper.py                # 범용 헬퍼 함수
├── common_shadcn_style.py          # re-export bridge → constants/ui.py
...
# 또는 기존 경로 (re-export bridge 유지)
from core.common_helper import SQLiteDML, SQLiteDDL
```
...
실제 정의는 `core/constants/business.py`에 있으며, `core/common_config.py`는 하위 호환용 re-export bridge다.

```python
from core.common_config import plant_codes, plant_oeqg_dict  # 기존 경로 (유효)
# 또는
