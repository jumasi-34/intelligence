...
* **핵심 배치 규칙**: 모든 비즈니스 로직 함수는 이 레이어 내의 `df_*.py` 파일에만 존재해야 합니다.
...
프로젝트 내 파일, 클래스, 변수, 데이터베이스 객체 등의 상세 명명 기준은 **[L2-naming-convention.md](file:///home/jumasi/workstation/intelligence/rules/L2-naming-convention.md)**에서 단일 진실 공급원(SSOT)으로 전담하여 관리합니다.

* **레이어별 파일 명명 규칙**: `*_page.py`, `*_plots.py`, `df_*.py`, `*_query.py`/`q_*.py` 등 상세 템플릿과 예시는 [L2-naming-convention.md](file:///home/jumasi/workstation/intelligence/rules/L2-naming-convention.md)를 참조하십시오.
* **공통 코어 폴더 구조 (`app/core/`)**:
  ...
  - `app/core/db/` & `app/core/operate/`: DB 클라이언트 및 커넥터 인터페이스 (`client.py`, `db_client.py`)
...
flowchart TD
    ...
    Service["app/service/df_*.py (Data Processing)"]
    Query["app/queries/*_query.py (SQL assembly)"]
    DB_Client["app/core/operate/db_client.py"]
...
* **규제 사항**: 
  ...
  - UI는 무조건 서비스 레이어(`app/service/df_*.py`)의 함수를 게이트웨이로 삼아서만 데이터를 수집할 수 있습니다.

### ③ 서비스 레이어의 UI 종속성 제거 (No UI Library in Service)
* **규제 사항**:
  - 서비스 레이어(`app/service/df_*.py`) 내부에 `plotly`, `matplotlib`, `streamlit` 등의 시각화/UI 패키지를 임포트하여 차트 객체(`go.Figure`, `plt.Figure` 등)를 생성하거나 반환하는 행위는 불허합니다.
...
3. **세부 판정 가이드**: 상세 시나리오는 [context-preprocessing-boundary.md](file:///home/jumasi/workstation/intelligence/context/context-preprocessing-boundary.md) 문서를 기준 삼아 준수합니다.
...
2. **데이터베이스 물리적 커넥터 구역**: `core/db/client.py` 및 `core/operate/db_client.py`
