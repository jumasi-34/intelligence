...
```mermaid
flowchart TD
    Reviewer["Code Reviewer Agent<br>(정적 리뷰 가이드 제공)"] -->|"1. 아키텍처 검증 통과"| Evaluator["Quality Evaluator Agent<br>(하네스 구동 및 채점)"]
    Evaluator -->|"2. Pass (합격) 완료"| Gateway["최종 배포 게이트<br>(수동 병합 승인)"]
    Evaluator -->|"2. Fail (불합격) 통보"| Reviewer
```
